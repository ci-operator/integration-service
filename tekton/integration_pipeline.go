...
// ReplaceGitResolverUpdateMap replace the url and revision in resolverParams by the url and revision from snapshot
...
func itsResolverParamsToMap(params []v1beta2.ResolverParameter) map[string]string {
...
func ShouldUpdateIntegrationPipelineGitResolver(integrationTestScenario *v1beta2.IntegrationTestScenario, snapshot *applicationapiv1alpha1.Snapshot) bool {
	...
	if testResolverRef.Resolver != consts.TektonResolverGit {
		return false
	}

	annotations := snapshot.GetAnnotations()
	params := itsResolverParamsToMap(testResolverRef.Params)

	// if target revision sha differs from source revision sha we do not want to overwrite it
	return doTargetAndSourceRevisionParamsMatch(annotations, params)
}
...
func NewIntegrationPipelineRun(client client.Client, ctx context.Context, loader loader.ObjectLoader, logger h.IntegrationLogger, prefix, namespace string, integrationTestScenario *v1beta2.IntegrationTestScenario, snapshot *applicationapiv1alpha1.Snapshot) (*IntegrationPipelineRun, error) {
	...
	if ShouldUpdateIntegrationPipelineGitResolver(integrationTestScenario, snapshot) {
		logger.Info("using the integration test pipeline/pipelinerun from the pr of snapshot", "integrationTestScneario.Name", integrationTestScenario.Name, "snapshot.Name", snapshot.Name, "resourceKind", resourceKind)
		resolverParams = ReplaceGitResolverUpdateMap(snapshot, resolverParams)
	}
	...
	if strings.EqualFold(resourceKind, consts.ResourceKindPipeline) || resourceKind == "" {
		...
		if err != nil {
			logger.Error(err, "failed to get pipelinerun yaml from pipeline run Resolver", "resolver", resolver, "tektonResolverParams", tektonResolverParams)
			return nil, err
		}
...
// updates only existing parameters, doesn't create new ones
func (iplr *IntegrationPipelineRun) WithUpdatedPipelineGitResolver(snapshot *applicationapiv1alpha1.Snapshot) *IntegrationPipelineRun {
	params := getGitResolverUpdateMap(snapshot)
	...
	if !shouldUpdatePipelineGitResolver(iplr.Spec.PipelineRef, snapshot) {
		return iplr
	}
...
func (iplr *IntegrationPipelineRun) WithUpdatedTasksGitResolver(snapshot *applicationapiv1alpha1.Snapshot) *IntegrationPipelineRun {
	params := getGitResolverUpdateMap(snapshot)
...
func doTargetAndSourceRevisionParamsMatch(annotations map[string]string, params map[string]string) bool {
...
// shouldUpdatePipelineGitResolver checks if the task resolver should be updated based on the source repo
func shouldUpdatePipelineGitResolver(pipelineRef *tektonv1.PipelineRef, snapshot *applicationapiv1alpha1.Snapshot) bool {
	// Don't modify anything if the pipelineRef doesn't use a git resolver
	//nolint:staticcheck  // QF1008: We specifically want ResolverRef.Resolver, not PipelineRef.Resolver
	if pipelineRef.ResolverRef.Resolver != consts.TektonResolverGit {
		return false
	}

	annotations := snapshot.GetAnnotations()
	//nolint:staticcheck  // QF1008: We specifically want ResolverRef.Params, not PipelineRef.Params
	params := tektonResolverParamsToMap(pipelineRef.ResolverRef.Params)
...
func shouldUpdateTaskGitResolver(task *tektonv1.PipelineTask, snapshot *applicationapiv1alpha1.Snapshot) bool {
	...
	annotations := snapshot.GetAnnotations()
	//nolint:staticcheck  // QF1008: We specifically want ResolverRef.Params, not PipelineRef.Params
	params := tektonResolverParamsToMap(task.TaskRef.ResolverRef.Params)

	return doTargetAndSourceRevisionParamsMatch(annotations, params)
}

func tektonResolverParamsToMap(params []tektonv1.Param) map[string]string {
	result := make(map[string]string)
	for _, param := range params {
...
// GenerateCleanData generate the clean yaml string used by UT integration_pipeline_test.go and snapshot_adapter_test.go
func GenerateCleanData(yamlStr string) string {
	re := regexp.MustCompile(`[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]`)
	cleanContent := re.ReplaceAllString(yamlStr, "")
	return base64.StdEncoding.EncodeToString([]byte(cleanContent))
}

// ConvertStringToPipelinerun convert string to pipelinerun object used by UT integration_pipeline_test.go and snapshot_adapter_test.go
func ConvertStringToPipelinerun(yamlStr string) (tektonv1.PipelineRun, error) {
	var pipelineRun tektonv1.PipelineRun
